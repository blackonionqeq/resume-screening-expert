...
**`src/providers.ts`** — Provider factory. Anthropic uses `@ai-sdk/anthropic`. OpenAI uses `@ai-sdk/openai`. DeepSeek reuses `createOpenAI` from `@ai-sdk/openai` with `baseURL: 'https://api.deepseek.com/v1'` (OpenAI-compatible API). Default models: `claude-sonnet-4-6`, `gpt-4o`, `deepseek-chat`.
